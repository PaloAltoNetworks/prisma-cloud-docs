== Bucket ACL grants FULL_CONTROL permission to everyone
...
An S3 bucket that allows FULL_CONTROL (READ, WRITE, READ_ACP, WRITE_APC) access to everyone can provide attackers the ability to add, delete and replace objects within the bucket, which can lead to: S3 data loss, unintended changes to applications using that bucket, and unexpected charges.
...
We recommend AWS S3 buckets are not publicly accessible for *FULL_CONTROL* actions to protect S3 data from unauthorized users and exposing sensitive data to public access.
