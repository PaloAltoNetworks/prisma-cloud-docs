...
+
CycloneDX format displays vulnerabilities or misconfiguration as a scheme outlined for XML version 1.4.
...
In this example, 20220801-221830_iac.csv. This file will display the following data for each IaC scan, including resources without misconfigurations:
...
Generating an SBOM in a CycloneDX format will generate a single file with sections like:
...
+
* *All*: This will generate three separate files of CSV, CycloneDX and a merge file of all resource types.
...
* *Images*: This includes packages in images built or references in the repository.
+
...
. Access the `checkov` command line in a specific directory or file. For example, `checkov -d . --bc-api-key PRISMA_ACCESS_KEY::PRISMA_SECRET_KEY` and th
