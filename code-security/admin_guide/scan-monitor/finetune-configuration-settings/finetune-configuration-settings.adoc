...
VCS pull request scans do not trigger notifications, because PR scans already get notifications in the PR itself in the form of comments and pipeline scans. However, if a PR gets merged and introduces new findings into the default branch, then those findings will generate alerts on the next periodic scan.
