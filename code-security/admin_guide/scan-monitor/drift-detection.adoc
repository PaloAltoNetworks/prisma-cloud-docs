...
If you would like to skip keys in our Drift Detection, you can leverage the native Terraform lifecycle.ignore_changes block. Differences for these key:values will not be marked as Drift in the platform. For example:
...
Will ignore differences in the value of tag "foo".
