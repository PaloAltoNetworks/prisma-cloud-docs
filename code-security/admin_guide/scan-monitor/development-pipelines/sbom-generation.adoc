...
. Add the format output syntax to the `checkov` line. For example, add `-o cyclonedx` to generate a CycloneDX XML SBOM, `-o cyclonedx_json` to generate a CycloneDX JSON SBOM, `-o spdx` to generate a SPDX SBOM and add `-o csv` to generate a CSV SBOM.
+
