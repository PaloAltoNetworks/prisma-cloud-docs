...
The Code Editor is a suitable option when you want to create  complex custom policies that include both Attribute and Connection-State with a support of AND/OR logic.
...
+
In this example, `aws_lb` and `aws_elb` must have connection with `aws_security_group` or `aws_default_security_group` to be compliant.
+
[source]
definition:
...
+
Code Editor appears as a default view. You can view an example of a YAML template.

.. Select *Clear Editor* to remove the example YAML template.
+
image::code-editor-1.png[width=600]

.. Add your custom YAML code.

.. Select *Test* to validate your custom code.
+
image::code-editor-2.png[width=600]
+
If your custom code has no error, Prisma Cloud will display 30 Test Results. This indicates Prisma Cloud has run a scan across all your repositories and has found upto 30 resources that are impacted by your custom code.
+
image::code-editor-3.png[width=600]
+
Select any test result to view more details on the impacted resource and misconfiguration.
+
image::code-editor-4.png[width=600]
+
However, if your code does have an error, the console will display a solution for the error.
+
