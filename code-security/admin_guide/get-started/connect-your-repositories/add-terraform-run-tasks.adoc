...
+
Access *Code Security > Projects* to view the latest integrated Terraform Cloud (Run Tasks) repository to xref:../../scan-monitor/monitor-fix-issues-in-scan.adoc[Suppress] or xref:../../scan-monitor/monitor-fix-issues-in-scan.adoc[Fix] the policy misconfigurations.