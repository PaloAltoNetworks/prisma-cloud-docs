...
+
For VScode (Visual Studio Code), get the details for enabling authentication to Prisma Cloud. Create Access Key for the IDE user with the Developer (minimum) role access to a specific repository. Developer role provides view and access to issue VCS “Fix and Submit” changes to the relevant repositories. A user can have more than one Prisma Cloud Role assigned to them. The default Role (assigned when the user is created or updated later) will be used when accessing Prisma Cloud with the user API Access Key. 
+
* *Access Key.*
+
