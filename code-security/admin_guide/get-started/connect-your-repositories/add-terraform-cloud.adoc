...
* `hard-mandatory` means your Terraform cannot be applied until you resolve or suppress all failing Code Security policies.
* `soft-mandatory` means your Terraform runs are blocked but can be overridden to still apply the IaC
* `advisory` means Code Security will report and record policy violations but will not block a Terraform apply.
+
