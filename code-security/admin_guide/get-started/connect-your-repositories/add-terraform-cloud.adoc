...
+
NOTE: If you are the owner of the workspace
...
+
Access *Code Security > Projects* to view the latest integrated Terraform Cloud (Sentinel) repository to xref:monitor-fix-issues-in-scan.adoc#suppress-scanned-resources[Suppress] or xref:monitor-fix-issues-in-scan.adoc#fix-scanned-resources[Fix] the policy misconfigurations.
