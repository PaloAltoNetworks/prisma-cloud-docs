== Bucket ACL grants READ_ACP permission to AWS users
...
An S3 bucket that allows *READ_ACP* access to AWS users can provide attackers the ability to read the bucket ACL and see who controls objects and how.
...
We recommend AWS S3 buckets are not accessible to ASW users for *READ_ACP* actions to protect S3 data from unauthorized users and exposing sensitive data to public access.
