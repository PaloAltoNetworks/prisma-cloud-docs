...
An S3 bucket that allows *WRITE_ACP* access to AWS users can provide attackers the ability to modify the ACL for the applicable bucket, which can lead to: S3 data loss, unintended changes to applications using that bucket, and unexpected charges.
...
We recommend AWS S3 buckets are not accessible to AWS users for *WRITE_ACP* actions to protect S3 data from unauthorized users and exposing sensitive data to public access.
...
See Procedure above for CLI commands to use to fix this policy error.
