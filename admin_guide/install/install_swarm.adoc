...
backend http_back
    balance roundrobin
    server node1 IP-OF-YOUR-SWARMWORKER:8081 check
...
. Open Prisma Cloud Console's web interface. The Web interface is available via HTTP (port 8081) and via HTTPS (port 8083) according to its configuration.
Go to \https://<LOAD-BALANCER>:8083 for HTTPS and \http://<LOAD-BALANCER>:8081 for HTTP.
+
