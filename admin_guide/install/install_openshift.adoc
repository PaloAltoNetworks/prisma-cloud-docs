...
Use the _twistcli_ tool to generate the Prisma Cloud Console deployment YAML.
The _twistcli_ tool is bundled with the release tarball.
There are versions for Linux, macOS, and Windows.

The _twistcli_ tool generates YAML for a ReplicationContoller, and other service configurations, such as a PersistentVolumeClaim, SecurityContextConstraints, and so on.
Run the twistcli command with the _--help_ flag for additional details about the command and supported flags.

You can optionally customize _twistlock.cfg_ to enable additional features, such as xref:../compliance/extensible_compliance_checks.adoc[custom compliance SCAP scanning].
...
When generating the Console deployment YAML files with _twistcli_, specify the name of the storage class with the _--storage-class_ flag.
...
Use the _twistcli_ tool to generate the DaemonSet deployment YAML.
The command has the following basic structure
It creates a YAML file named _defender.yaml_ in the working directory.

  $ <PLATFORM>/twistcli defender export openshift \
    --address <ADDRESS>
    --cluster-address <CLUSTER-ADDRESS>

The command connects to Console’s API, specified in _--address_, to generate the Defender DaemonSet YAML config file.
...
include::fragments/install_defender_twistcli_export_oc.adoc[leveloffset=+1]
endif::prisma_cloud[]
...
* Deploy Console and Defender with _helm install_ rather than _oc create_.
...
* Deploy Defender with _helm install_ rather than _oc create_.
...
. Unpack the chart into a temporary directory.
  ...
  $ mkdir helm
  $ tar xvzf twistlock-console-helm.tar.gz -C helm/

. Open _helm/twistlock-console/templates/securitycontextconstraints.yaml_ for editing.
...
. Repack the Helm chart, and install it in your OpenShift 4 cluster.
