...
For more information about Console's minimum requirements, see the xref:../install/system_requirements.adoc[system requirements].
...
+
NOTE: If this role doesn't exist, see https://docs.aws.amazon.com/AmazonECS/latest/developerguide/instance_IAM_role.html[Amazon ECS Container Instance IAM Role].
...
For more information, see the xref:../install/system_requirements.adoc[system requirements].
...
If you already have worker nodes in your cluster, skip directly to <<_create_defender_task_definition,creating the Defender task definition>>.
...
[#_create_defender_task_definition]
[.task]
...
+
Console will automatically retrieve the cluster name from AWS.
