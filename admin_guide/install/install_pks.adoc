...
https://cloud.vmware.com/vmware-enterprise-pks[VMware Enterprise PKS] lets you deploy Kubernetes clusters on demand.
...
In PKS, the Docker socket can be located in either _/var/vcap/data/sys/run/docker/docker.sock_ or _/var/vcap/sys/run/docker/docker.sock_.
