...
+
NOTE: In order to deploy a new Defender after the Prisma Cloud tile is installed (e.g. if new Diego cells or isolation segments have been installed in the TAS environment), *Apply changes* on Ops Manager is required. Note that the *Apply changes* action updates the existing Defenders to the version of the installed Prisma Cloud tile.
