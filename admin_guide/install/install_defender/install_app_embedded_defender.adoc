...
Besides runtime policy, you can also configure the xref:../../firewalls/cnaf.adoc[CNAF] application firewall to protect front-end containers.
...
App Embedded Defender has been tested on Azure Container Instances, DC/OS (unified container runtime) , PCF PAS, and Google Serverless containers
...
+
<USER>::
Name of a Prisma Cloud user with a minimum xref:../../access_control/user_roles.adoc[role] of Defender Manager.
