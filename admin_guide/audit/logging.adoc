...
All events associated with CNAF rules for container, hosts and app-embedded generate audits.

NOTE: CNAF serverless events are not registered in the syslog. Events audits will be registered to the syslog in future releases. 

NOTE: CNAF Container and Host rule audits are written to the Defender host's syslog. CNAF App-Embedded rule audits are written to the console's host's syslog. 

Message fields for CNAF audit would change based on the deployment type as follows:
