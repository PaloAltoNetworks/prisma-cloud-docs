...
[.section]
=== Firewall audits
Cloud Native Network Firewall (CNNF) utilizes Prisma Cloud runtime models to identify anomalous traffic flows between containers.
Anomalous traffic flows generate an audit, and could indicate that an attacker is moving laterally through your environment.
Cloud Native Application Firewall (CNAF) is a layer 7 filtering engine that ensures only safe, clean traffic ever reaches your web app.
Audits are generated when CNNF detects an attack, such as SQL injection or cross-site scripting.
...
Only users with Administrator, Operator, Defender Manager, or Auditor xref:../access_control/user_roles.adoc[roles] can view audit data in Console.
