...
xref:../authentication/user_roles.adoc#[User roles], and xref:../authentication/access_keys.adoc[access keys].
...
+
This is the email address and the access key of the user with the CI role (Build and Deploy Security permission group with the option to create an access key on Prisma Cloud). 
