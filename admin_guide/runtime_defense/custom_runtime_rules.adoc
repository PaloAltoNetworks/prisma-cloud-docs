...
Custom rules are written and managed in Console under *Defend > Runtime > Custom Rules*.
...
[.procedure]
. In Console, go to *Defend > Runtime > Custom Rules*.
...
Be sure that there is no conflict between your custom rules and other settings in your runtime rule, such as whitelists and blacklists.
...
. Specify an effect for each expression.
+
image::custom_runtime_rules_effect.png[width=600]
...
The same logic applies to custom rules that try to whitelist processes that are prevented by other policies.
For example, consider process 'foo' that is an explicitly denied by a runtime rule, with the effect set to *Prevent*.
You cannot whitelist 'foo -bar' in a custom runtime rule by analyzing proc.cmdline for for '-bar'. 
