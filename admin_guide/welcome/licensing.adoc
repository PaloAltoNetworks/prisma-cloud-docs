...
Container Defender protects both the underlying host and any containers it runs, and the license (7 workloads) includes coverage for both.
A container host consumes 7 workloads whether it runs one container or a hundred containers.
...
*Example*: Assume you've licensed 700 workloads to cover 100 container hosts, and usage fluctuates from week to week:

Nov 1-7: Lower demand, uses 90 nodes (630 workloads)
Nov 8-15: Uses 100 nodes (700 workloads)
Nov 16-22: Uses 100 nodes (700 workloads)
Nov 23-30: High demand, uses 110 nodes (770 workloads)

Even though you used 770 workloads for a short period of time, you're still properly licensed because the 30 day rolling average is 700:

(630 + 700 + 700 + 770) / 4 = 700 workloads
...
You would procure a license with 700 workloads:

100 container hosts * 7 workloads per container host = 700 workloads

Serverless functions are licensed based on the number of invocations, and averaged over the period of a month.
Every 1 million serverless invocations in a one month period counts as 1 workload.

*Example*: Assume you have 5 functions and, in total, they are invoked 8 million times over the period month.
You would procure a license with 8 workloads.
...
You would procure a license with 120 workloads.
