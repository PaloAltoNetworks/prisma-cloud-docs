...
ServiceNow short description fields list the resource.
ServiceNow long description fields list the details of each finding.
...
ServiceNow's priorities map one-to-one to Prisma Cloud priorities (Critical - Critical, High - High, Medium - Medium, Low - Low).

Compliance: max severity from the resource’s (image/container/host) new compliance issues.
ServiceNow's priorities map one-to-one to Prisma Cloud priorities (Critical - Critical, High - High, Medium - Medium, Low - Low).
...
Prisma Cloud Compute audit - <audit type>
