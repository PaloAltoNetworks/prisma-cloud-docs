...
NOTE: Immediate alerts is not supported for registry scan vulnerabilities

Send vulnerability immediate alerts can be configured per alert profile and is disabled by default.

The immediate alerting won't affect the 24 hours Vulnerabilities report. The report will include all vulnerabilities that were detected in the last 24 hours including those which were sent via an immediate alert.
