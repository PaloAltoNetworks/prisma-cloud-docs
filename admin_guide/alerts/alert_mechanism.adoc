...
You can also choose to send an immidiate alert when a new vulnerability was detected on a new deployed resource or when scanning a  containers and hosts. Sentind a vulneravility immediate alerts can be configured per alert profile, and is disables by defult.

You can also choose to send immidiate alert when detecting new vulnerabilities:
[. procedure]
. Deploy a new image/host with vulnerabilities
. Detect new vulnerabilities in the scan of existing image/host 
