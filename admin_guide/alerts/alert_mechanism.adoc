...
NOTE: Immediate alerts is not supported for registry scan vulneravilities

Sentind vulneravility immediate alerts can be configured per alert profile, and is disables by defult.

The immideate alerting won't affect the 24 hours Vulnerabiliies report. The report will include all vulnabilities that were detected in the last 24 hours including those which were sent via an immideate alert.
