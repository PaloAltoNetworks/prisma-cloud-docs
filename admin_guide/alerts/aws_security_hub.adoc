...
* Set *Use IAM Role* to *On* to use the xref:../configure/credentials_store.adoc#_aws_iam_users[role assigned to the EC2 instance] that runs Console.

* Select a *Credential*, which holds the access key and secret for a service account.
Select an existing credential or xref:../configure/credentials_store.adoc#_aws[create a new one].

. (Optional) Toggle the *Use AWS STS* to *On*.
In *Role ARN*, specify the role to use.
+
For more information about AWS STS, see xref:../configure/credentials_store.adoc#_aws_security_token_service_sts[AWS Security Token Service].
