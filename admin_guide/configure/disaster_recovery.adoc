...
* *Console* --
Go to *Manage > System > Backup & Restore*, and click *Actions > Export* to download a backup.

* *API* --
Use the `GET /api/v1/backups/{id}` endpoint to download a specific backup, where `{id}` is the full name of the backup file.
An example of a full file name is `daily-20.03.140-1583278547.tar.gz`.
Full file names can be retrieved from Console's data folder here: `/var/lib/twistlock-backup`.
