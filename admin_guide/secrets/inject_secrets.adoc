...
To inject secrets into your containers, first xref:../secrets/integrate_with_secrets_stores.adoc#[integrate Prisma Cloud with your secrets manager], and then set up rules for injecting specific secrets into specific containers.
...
. Specify a filter for your rule.
By default, all secrets are injected into all containers on all hosts.
