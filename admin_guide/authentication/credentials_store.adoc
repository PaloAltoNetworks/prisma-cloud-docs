...
4+|Alerting -- GCP Security Command Center
4+|Alerting -- Google Cloud Pub/Sub

|===

^1^N/A means the feature isn't supported in Prisma Cloud for given cloud provider.
Creating service account or role would serve no purpose.
...
The kubeconfig for these services require external binaries for authentication (Google Cloud SDK and aws-iam-authenticator respectively), and Prisma Cloud Console doesn't ship with these binaries.
