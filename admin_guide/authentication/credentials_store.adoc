...
.. In the *Credential level* field, select the access level of your service account. For service account with permission for a single project selet *Project*. For service account with permissions for an organization or multiple projects, select *Organization*. With organization-level credentials, Prisma Cloud will find and scan all the projects in the organization.
+
...
+
NOTE: For organization-level credentials, the performance of some features may be affected if the organization includes a large number of projects. For specific information and guidelines see xref:../vulnerability_management/registry_scanning/scan_gcr.adoc[Google Container Registry (GCR) scanning] and xref:../install/install_defender/install_cluster_container_defender.adoc[managed Defender DaemonSet deployment for GKE].
