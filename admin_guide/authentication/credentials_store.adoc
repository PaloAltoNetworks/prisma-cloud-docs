...
For all other integrations, manually create the required service accounts or roles according to the https://cdn.twistlock.com/docs/downloads/Compute-Feature-Wise-Permissions.pdf[feature-wise permissions doc].
...
You can find full list of feature wise AWS permissions here: http://cdn.twistlock.com/docs/downloads/Compute-Feature-Wise-Permissions.pdf and add them manually.


===== Cloud Account Permission Status

CFTs will be updated with Compute permissions on platform level on tentatively Oct 7th for all accounts but the status check on those permissions for Compute will NOT be done by default, the following criteria must be met:
        
* The tenant must have atleast 1 average Compute workload for the status permission check to change to orange with missing permissions on Compute.
* The status check for average (30 day workload consumption)will be done every 24 hours for enabling Compute permissions check.
