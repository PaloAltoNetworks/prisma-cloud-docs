...
NOTE: Changes to role permissions while users are logged in to it will only apply after users re-login.
...
[NOTE]
====
If a user is assigned multiple system roles, either directly or through group inheritance, then he is granted the rights of the highest role. 
ifdef::compute_edition[]
If a user is assigned both system and custom roles, then he will be randomly granted the rights of one of the roles.
endif::compute_edition[]
