...
Integration with older versions should work as well, but is not officially supported.
...
+
If the log in is successful, you are directed to a view appropriate for the user's role.
Users with the User role are directed to a single page, *Configure > Clients*, from where they can download the certs required to access a Prisma Cloud-protected container environment.
