...
* *Penalty Box for Attackers* - CNAF supports a 5 minutes ban of IPs triggering one of it’s protections to slow down vulnerability scanners and other attackers probing the application.

NOTE: Currently, CNAF and Segment cannot interoperate simultaneously on the same node.
If Segment is enabled in a Defender, CNAF will not be available to protect the workloads on that same node.
...
CNAF's management console is independent of the Defenders and can be self-hosted or provided as a service (SaaS):
...
CNAF is enabled by xref:./deploy_cnaf.adoc[adding a new CNAF rule] onto xref:../install/install_defender/install_defender.adoc[deployed Defenders].
...
CNAF inspects all HTTP requests sent to the application and protects against all types of injection attacks as well as local file inclusions (see more on Local File Inclusion below).
...
CNAF checks for requests that are crafted to exploit the Shellshock vulnerability.
...
* *_Denied inbound IP Sources_* - CNAF applies selected action (Alert or Prevent) for IP addresses in network lists
...
* *_Denied Inbound Source Countries_* - CNAF applies selected action (Alert or Prevent) for requests originating from the specified country.
