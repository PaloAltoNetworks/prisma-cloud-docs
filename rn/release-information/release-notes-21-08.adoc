...
For maximum stability, use these APIs for integrations and automation scripts. With the backward compatibility support for n-2 releases, you will need to update your scripts every three releases to ensure compatibility with the product release lifecycle.
* The v1 version always points to the latest API and you can continue to use the v1 APIs. 
To ensure that your scripts continue to work as expected, you must review the API change notifications for changes and deprecations and update your implementation to keep up-to-date with the latest release. 
