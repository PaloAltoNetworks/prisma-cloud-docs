...
Kubernetes auditing is now supported for AWS EKS clusters and Azure AKS clusters to help you identify risks and security events.
The configuration settings on *Defend > Access > Kubernetes* is enhanced to include three cloud providers- GCP, AWS, and Azure. 

Additionally, you can configure Kubernetes auditing policy rules using a cluster filter and apply rules to specific clusters, while continuing to ingest data for a larger set of clusters.  
