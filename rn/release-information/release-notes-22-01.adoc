...
* The Runtime events may include sensitive information that's found in the running commands. Now you can filter this sensitive information and ensure that it is not included in the Runtime findings (including Forensics, Incidents, audits, etc.).
     ...
     ** There are two options to scrub your sensitive Runtime data in Prisma Cloud Compute:
          *** Default scrubbing configuration: automatically scrub secrets from runtime events. This configuration will be *emabled* by default when upgrading the Console.
          *** Customize your own regex to detect and scrub sensitive information, in advance of the existing capabilities in WAAS.
     ** Head to Manage > General to see the options now available.
    
==== Vulnerabilities immediate alerts

* You can now choose to send alerts immediately when new vulnerabilities are detected, this is in advance of the existing vulnerabilities alerting mechanism. 
* The immidiate alert can be sent in one of the following scenarios:
