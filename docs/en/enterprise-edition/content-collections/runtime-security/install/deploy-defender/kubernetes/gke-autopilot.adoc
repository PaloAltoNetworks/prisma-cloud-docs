...
GKE Autopilot clusters are using https://cloud.google.com/kubernetes-engine/docs/concepts/using-containerd[*cos_containerd*] nodes, therefore the DaemonSet must  be configured with *Containerd*. 
...
[NOTE]
====
If you are using the web interface, on *Runtime Security > Manage > Defenders > Deployed Defenders > Manual deploy* ensure that the *orchestrator type* is *Kubernetes*, select the *Container Runtime type* as *Containerd*, and enable *GKE Autopilot deployment*.
====
