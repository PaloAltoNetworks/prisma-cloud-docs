...
image::runtime-security/waas-response-custome-rule-type.png[]
====
...
The WAAS events for JWT custom rule violations are generated under *Runtime Security > Monitor > Events > WAAS for containers/hosts*, with the attack type as *Custom Rule*.

image::runtime-security/waas-events-jwt.png[]
...
[.procedure]
. Open Console, and go to *Runtime Security > Defend > WAAS* > *{Container | Host | App-Embedded | Agentless} >  {In-line | Out-Of-Band}*.
...
+
image::runtime-security/waas-custom-rule-regex-match-group.png[]
...
Custom rules are defined in *Runtime Security > Defend > Custom rules > WAAS*.
WAAS policy rules are defined in *Runtime Security > Defend > WAAS > {Container | Host | App-Embedded | Agentless} > {In-line | Out-Of-Band}*.
...
*Prerequisites:* You have already set up WAAS to protect an app, and there's a rule for it under *Runtime Security > Defend > WAAS > {Container | Host | App-Embedded | Agentless} > {In-line | Out-Of-Band}*.
...
+
In addition to selecting the global effect for applying the virtual patch, you can also override the default effects by selecting *user-selected custom rules* that are always applied regardless of the global auto-apply virtual patch under *Runtime Security > Defend > WAAS > Container/Host/Agentless/App-embedded > {In-Line/Out-Of-Band}*.
+
...
+
image::runtime-security/waas-custom-rules-min-defender.png[]
