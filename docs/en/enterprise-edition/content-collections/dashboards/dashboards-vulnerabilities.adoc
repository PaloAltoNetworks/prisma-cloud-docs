...
- **Top Impacting Vulnerabilities** - Provides a ranked list of the most critical vulnerabilities in your environment based on the risk score. The ranked list consists of CVEs that are affecting the environment. Each CVE includes data about its risk factors, severity, CVSS, risk factors, and assets impacted.
+
...
- **Vulnerability Impact by Stage** - Visualize the sources of the vulnerabilities and the impact of the vulnerability across app stages of your application lifecycle.
+
...
.. Select a vulnerability from any of the entry path on the dashboard. Refer to the possible entry paths in xref:prioritize-assess-risk[prioritize and assess risks].
