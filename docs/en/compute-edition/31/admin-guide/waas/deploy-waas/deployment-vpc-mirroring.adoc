...
.. Select the credentials for the AWS *Cloud account* that you configured under *Manage > Cloud accounts*.
...
. Go to *Monitor > Events > WAAS for Agentless* to observe the events generated. 
+
...
image::waas-agentless-rules.png[width=200]
...
+
NOTE: You must upgrade the VPC Observer through *Manage > Defenders*.
