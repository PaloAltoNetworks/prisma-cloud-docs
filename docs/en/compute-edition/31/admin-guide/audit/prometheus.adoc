...
For a thorough explanation of this type of metric, see xref:../vulnerability-management/vuln-explorer.adoc#roll-ups[Vulnerability Explorer].
...
If you want to use Prometheus with xref:../deployment-patterns/projects.adoc[projects], modify the scrape configuration file with an additional job for each Prisma Cloud Console.
