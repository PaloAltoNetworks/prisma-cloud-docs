...
. Use the <<_script,enhanced script>> in place of the standard script you copy from Console when <<_configuring_servicenow,setting up the Scripted REST API>>.
...
Use it in place of the script you copy from Console when setting up the Scripted REST API in <<_configuring_servicenow>>.
To use the script, you must first set up a <<_map_vuln_to_ci,discovery source and identification rule>>.
