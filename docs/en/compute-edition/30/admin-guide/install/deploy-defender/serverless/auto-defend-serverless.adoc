...
It is an additional option for deploying the Serverless Defender, on top of manually adding it xref:serverless.adoc[as a dependency] or adding it xref:install-serverless-defender-layer.adoc[as a Lambda layer].
