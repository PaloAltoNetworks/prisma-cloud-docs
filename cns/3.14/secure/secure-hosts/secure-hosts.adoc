...
By default, Aporeto recognizes only containers on the host as xref:../concepts.adoc#processing-units[processing units].
...
* xref:./host/[Protecting other hosts]
* xref:./k8s/[Protecting Kubernetes hosts]
* xref:./openshift/[Protecting OpenShift hosts]
