...
{{< tip >}}
If you're not sure if the cluster uses a CNI plug-in or not, `exec` into one of your nodes and run `cat /etc/cni/net.d`. 
...
[TIP]
====
We detail the `apoctl enforcer install kubernetes` command further in the xref:../../apoctl.adoc#kubernetes-subcommand[reference documentation].
