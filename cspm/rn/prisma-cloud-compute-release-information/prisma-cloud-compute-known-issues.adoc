...
//CWP-51415
|tt:[Fixed in 31.02.xxx]
|Fixed issue in RHEL clusters running NodeOS where compliance checks didn't show any non-compliant alerts.

//CWP-51013
|tt:[Fixed in 31.02.xxx]
|Fixed an issue where Defender failed to block containers when containers where restarted frequently and generated too many audit events. This was fixed by decreasing the number of requests sent to the Console.
