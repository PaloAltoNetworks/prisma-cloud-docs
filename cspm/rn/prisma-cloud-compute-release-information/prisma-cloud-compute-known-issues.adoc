...
|Due to a bug in Azure API, disks created by the agentless scanning process could potentially remain dangling (unattached). These disks are created with “[Delete with VM|https://learn.microsoft.com/en-us/azure/virtual-machines/delete?tabs=portal2%2Ccli3%2Cportal4%2Cportal5]” option which should ensure that they are deleted once the scanner VM has terminated.
