...
You can now directly add GitLab Container Registry in the `POST, settings/registry` endpoint by using the new schema instead of adding it through Docker V2 registry which requires administrative permissions.
