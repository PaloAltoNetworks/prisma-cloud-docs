...
|*Added Support of Registry Tags directly in Compute Collections*
...
This enhancement facilitates the association between the registry and the scanned images pertaining to that registry.
This change enables registry-based role-based access control (RBAC) for improved security and management.

|*Added Support for Continuous Integration (CI) scanning of images on Linux using containerd*
...
|*Added Support for GKE CIS benchmark Compliance Checks for worker nodes*
|Support the most recent version of the CIS Benchmark for Google Kubernetes Engine (GKE), specifically version 1.4.0.
This update includes checks for worker nodes, enhancing our compliance scanning capabilities.
