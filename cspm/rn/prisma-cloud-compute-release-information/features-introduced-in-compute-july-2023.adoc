...
|Added support for https://docs.paloaltonetworks.com/prisma/prisma-cloud/prisma-cloud-admin-compute/vulnerability_management/registry_scanning/scan-gitlab[scanning GitLab Container Registry] without usings admin credentials to manage and get a full list of all container registries/images.
...
|Add fields to enhance the syslog output in alignment with the the API call responses. The time field is replaced with the incident time field and the should_collect field is removed.
