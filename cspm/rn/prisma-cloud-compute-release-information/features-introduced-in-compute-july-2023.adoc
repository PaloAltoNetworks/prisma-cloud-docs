...
|Add fields to enhance the syslog output in alignment with the API call responses. The `time` field is replaced with the `incident time` field and the `should_collect` field is removed.
