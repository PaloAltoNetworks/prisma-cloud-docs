...
The following table lists the known and addressed issues on Prisma Cloud for the CSPM, Compute , and Application Security capabilities.
...
|Due to a bug in Azure API, disks created by the agentless scanning process could potentially remain dangling (unattached). These disks are created with "Delete with VM" option which should ensure that they are deleted once the scanner VM has terminated.
