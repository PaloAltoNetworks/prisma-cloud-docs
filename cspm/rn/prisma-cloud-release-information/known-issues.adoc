...
|The behavior of filters on the *Alerts* page is different from that on the *Asset Inventory* and *Asset Explorer* pages. On the *Alerts* page when you select multiple filters such as, userinput:[Asset Class], userinput:[Resource Type], and userinput:[Service Name], the alerts displayed will be a combination of the 3 selected filters. Whereas on the *Asset Inventory* and *Asset Explorer* pages, the filter on the assets pages gives preference to userinput:[Resource Type] over userinput:[Service Name] when both filters are selected and the alerts displayed will not match those displayed on the *Alerts* page.


|*RLP-86596*
tt:[Fixed in 23.2.1]

|With SSO enabled, the permissions granted to your authorized users via IdPs cannot be modified in the Prisma Cloud UI or via APIs.
