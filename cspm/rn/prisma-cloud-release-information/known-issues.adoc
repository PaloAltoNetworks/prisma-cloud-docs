...
|Clicking an S3 bucket on the *Data Settings* page would redirect to an incorrect assetID and would display *Asset Not Found* error.
This issue has been fixed.

|*RLP-98082*
//Raised in 23.4.1
|PCDS Azure only—Prisma Cloud is not able to create event grid subscriptions on storage accounts in a few regions due to which it cannot do forward scan on those storage accounts.
