...
|The behavior of filters on the *Alerts* page is different from that on the *Asset Inventory* and *Asset Explorer* pages. On the *Alerts* page when you select multiple filters such as, screen:[Asset Class], screen:[Resource Type], and screen:[Service Name], the alerts displayed will be a combination of the 3 selected filters. Whereas on the *Asset Inventory* and *Asset Explorer* pages, the filter on the assets pages gives preference to screen:[Resource Type] over screen:[Service Name] when both filters are selected and the alerts displayed will not match those displayed on the *Alerts* page.
