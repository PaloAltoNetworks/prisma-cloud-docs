...
A code fix was implemented on October 3, 2022, and we are validating the final results for impacted policies. The impacted policies are tagged with a label named *Policy Status Review*, and you can find more details  https://docs.paloaltonetworks.com/content/dam/techdocs/en_US/pdf/prisma/prisma-cloud/prerelease/prisma-cloud-policy-status-review.pdf[here]. 
