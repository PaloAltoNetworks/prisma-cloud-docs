...
*Impact—* Medium. New alerts might be generated in case the vulnerability assessment is disabled. Existing alerts might get resolved as vulnerabilityAssessments[*].properties.storageContainerPath does not exist.
...
*Impact—* Low - Alerts will be resolved for EC2 instance which are in inactive state.
