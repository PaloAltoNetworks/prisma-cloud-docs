...
*Impact—* Low. Existing alerts where the cluster is configured with ADVANCED_DATAPATH will be resolved as *Policy_Updated* and new alerts will be generated where it checks for ‘Calico Kubernetes Network policy’ not being configured.
