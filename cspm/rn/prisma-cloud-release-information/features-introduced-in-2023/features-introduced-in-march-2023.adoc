...
//RLP-89582 - Jen to share image and link
...
|If you are using tags, you no longer need to use a delimiter (colon :) to separate key and value entries in a single text box while assigning resource tags on the *Asset Inventory* and *Alert Overview*. You can now enter *Key* and *Value* in separate text boxes.
...
* *Publicly exposed EC2 instance which is not configured with Metadata Service v2 (IMDSv2) and is attached to an AWS role with s3:GetObject permission*

This policy identifies AWS EC2 instances which are publicly exposed and not configured with Instance Metadata Service v2 (IMDSv2). With IMDSv2, every request is protected by session authentication. IMDSv2 protects against misconfigured-open website application firewalls, misconfigured-open reverse proxies, unpatched SSRF vulnerabilities, and misconfigured-open layer-3 firewalls and network address translation. As a best practice, only use IMDSv2 for all your EC2 instances. 

* *Publicly exposed EC2 instance which is attached to an AWS role with iam:PassRole and ec2:RunInstances permissions*

This policy identifies AWS EC2 instances which are publicly exposed and are attached to an IAM role with risky permissions. EC2 instances associated with 'iam:PassRole','ec2:RunInstances' permissions can be used to escalate privileges by passing an existing IAM role to a new EC2 instance and moving laterally. It is highly recommended that you remove the risky permissions from the IAM role attached to EC2 instances. 

* *Publicly exposed EC2 instance which is attached to an AWS role with org level write permissions*

This policy identifies AWS EC2 instances which are publicly exposed and are attached to an IAM role with risky ORG level WRITE permissions. EC2 instances having org level write permissions can be used to escalate privileges at the ORG level and move laterally between accounts. It is highly recommended to remove the risky permissions from the IAM role attached to EC2 instances. 

* *Publicly exposed EC2 instance with a Critical/High exploitable vulnerability*

This policy identifies AWS EC2 instances which are publicly exposed and have known exploitable vulnerabilities. An attacker can exploit the vulnerability to compromise the confidentiality, integrity, or availability of the affected EC2 instance and perform malicious actions. As a best practice, remediate the Critical/High exploitable vulnerabilities reported for EC2 instances. 

* *Publicly exposed EC2 instance which is attached to an AWS role with iam:PassRole and lambda:InvokeFunction permissions*

This policy identifies AWS EC2 instances which are publicly exposed and are attached to an IAM role with risky permissions. EC2 instances having 'iam:PassRole','lambda:CreateFunction', 'lambda:InvokeFunction' permissions can be used to escalate privileges by passing an existing IAM role to a new Lambda function and moving laterally. As a best practice remove the risky permissions from the IAM role attached to EC2 instances. 
...
|Command Center > Vulnerability Swimlane Summary API

|*Place holder*
//RLP-92656
|AWS Automation flow with new onboarding apis
