...
|DESCRIPTION
// Place holder
|*Change to existing AssetExplorer API*

+++<draft-comment>RLP-86136</draft-comment>+++
|A new query parameter screen:[includeEventForeignEntities] is added to the existing screen:[GET/resource/scan_info] endpoint.

The query parameter is only applicable for compliance flows because the Asset Inventory only counts assets that belong to your cloud account. Foreign entities, such as SSO or Federated Users, are included in the Compliance Dashboard. These are resources that are not ingested directly from the monitored cloud accounts.
