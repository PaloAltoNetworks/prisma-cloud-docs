...
//RLP-78932
|The Adoption Advisor is now accessible for all Prisma Cloud users. Based on your role and access privileges, you can view a list of items and widgets that provide visibility into your operationalization journey and guidance on the next steps and remediation actions to secure your cloud infrastructure from code to cloud.

|tt:[Enhancement] *IAM Asset Details*
//RLP-97778
|Routine IAM access review is a key component of managing threats to your cloud environment. Enhancements to  "Inventory > Asset Explorer > Asset Detail View" provide you with greater visibility into the IAM access information associated with your assets. Currently, additional information is only available for AWS: 
...
*Impact—* Medium. New alerts are generated for the failing resources. This includes resources where Master authorized networks was previously enabled but are now configured as disabled.
...
*Impact*— Low. Previously generated alerts is resolved as *Policy_Deleted*. The out-of-the-box compliance mappings for the above policies are be removed and can affect the compliance score.
