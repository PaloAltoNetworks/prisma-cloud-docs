...
* After configuring data security for your Azure Subscription account, if you see missing permissions error message, it is because you have enabled access to storage account from a selected network and IP address. To fix this issue, add the following IPs to the firewall allow list:
