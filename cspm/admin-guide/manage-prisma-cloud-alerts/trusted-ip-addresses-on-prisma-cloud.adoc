...
.. Enter the *Trusted List Name* and (tt:[optional]) *Description*.

.. Select the Anomaly Policy for which you want alerts to be triggered when they detect suspicious domains in DNS queries.
+
image::add-domain-trusted-list-dns-1.png[scale=30]

.. *Next*.

..Enter one or more *Domains* and (tt:[optional]) *Account ID* from the drop-down list.
+
You can select only one Account or set it to *Any* to exclude any account that is added to Prisma Cloud.
+
image::add-domain-trusted-list-dns-2.png[scale=30]
