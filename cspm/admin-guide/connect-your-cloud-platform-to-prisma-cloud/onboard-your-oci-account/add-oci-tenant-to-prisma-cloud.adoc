...
[NOTE]
====
You can use an existing user with the correct privileges, an existing group, and an existing policy with the correct policy statements. It is, however, recommended that you create a new user, group, and policy as described in Step 3: *Create a User to Enable Access*.
====
+
...
+
*OCI has a limit of 50 policy statements. With the addition of new APIs, Prisma Cloud will now contain 56 policy statements in the Terraform file. To successfully ingest the new OCI APIs, request a https://docs.oracle.com/en-us/iaas/Content/General/Concepts/servicelimits.htm#[service limit increase] on the policy statements before running the Terraform file.*
+
...
.. From the Job details, navigate to "Resources > Outputs", copy user_ocid, and add it to Prisma Cloud.
+
