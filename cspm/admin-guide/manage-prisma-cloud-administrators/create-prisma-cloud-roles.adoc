...
* Select *Developer* role to enable limited access to developers who use the Code Security scanning capabilities. The developers who are assigned this role can generate access keys, and view IaC scan results on *Code Security* including the ability to fix issues.
