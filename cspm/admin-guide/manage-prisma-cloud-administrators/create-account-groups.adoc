...
To add one or multiple Cloud Accounts under a Parent Cloud Account in a Account Group:

[.procedure]
. If the Account Group do not exist, create one. Follow the same steps as below.

. Select menu:Settings[Account Groups].

. To edit the details of an Account Group, click the edit icon for the corresponding record.

. Under the Select Cloud Accounts select Group By as Parent Account. It will list all the Parent Cloud Accounts added.
+
image::manage-account-groups-by-parent-account.png[]

. Search for the Parent Cloud Account you are interested by using the search bar.

. Expand the rows under the Parent Cloud Account by clicking on the record.

. Select one or more Cloud Accounts to be added to the Account Group.
+
