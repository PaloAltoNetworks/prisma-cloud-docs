...
[NOTE]
====
Just in Time (JIT) provisioning, used along with SSO automates the process of creating accounts for users on various applications. JIT is disabled by default. 
====
