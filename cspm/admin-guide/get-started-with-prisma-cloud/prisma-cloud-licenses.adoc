...
+
The Runtime Security Foundations includes the core capabilities of Visibility, Compliance and Governance (VC&G), Threat Detection and Agentless Security. 
+
The Runtime Security Advanced includes the core capabilities of Visibility, Compliance and Governance (VC&G), Threat Detection and Agentless Security along with Host Security, Container Security, and IAM Security.
+
Runtime Security Foundations and Runtime Security Advanced also includes the option to activate Infrastructure as Code (IaC) Security (formerly known as Code Security) with developer based metering. Available modules include: Software Composition Analysis (SCA), offered at 4 credits per Developer and Secrets Scanning.
