...
[NOTE]
====
Use the following syntax to find cross-account connections: screen:[config from iam where source.cloud.account != dest.cloud.account]
====
...
+
Lists all permissions associated with the "App Registration" entity type in Azure: screen:[ config from iam where source.cloud.resource.type = 'App Registration' and dest.cloud.type ='Azure'.]
...
+
Lists all  permissions granted by "Service Principal" entity type: screen[config from iam where dest.cloud.type = 'Azure'  AND grantedby.cloud.type = 'AZURE' AND grantedby.cloud.entity.type = 'Service Principal"] 
