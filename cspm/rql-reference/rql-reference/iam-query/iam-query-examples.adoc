...
|*Last & Inactive Accesss*
|


|Show all active identites access and specific actions over last period of days
|[userinput]
----
...
|Show inactive identities and their allowed actions over a period of days
|[userinput]
----
config from iam where dest.cloud.type = 'AWS' AND action.lastaccess.days > 90
----
...
|Show only permissions used in last period of days that are granted by a role.  This can also be done by other entity types such as 'group' which may have inline policies attached directly to the group.    This type of query can be very powerful to help create new least privilege Custom Roles/Policies by only looking at what permissions are actually being used in the current role/policy.
|[userinput]
----
