...
The userinput:[_Set] function enables you to compare the values between lists on the Left Hand Side and Right Hand Side using the properties of union or intersection, and identify whether a specific value or comma separated list of values are included within that result set. The methods supported are userinput:[_Set.intersection] and userinput:[_Set.union] , and you can use the boolean operators userinput:[intersects] and userinput:[contains] to verify whether the values you want to look for are included in the result or if the result set contains the specified value(s).
