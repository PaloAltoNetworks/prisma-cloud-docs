...
Checks if any binary file in the image matches the md5 checksum for known malicious software.
For this check, we use the database composed of our Intelligence Stream (IS) and WildFire. We check the CI images with the IS and WildFire, and deployed images only with the IS. You can populate md5 by adding them to the xref:../configure/custom_feeds#_create_a_list_of_malware_signatures_and_trusted_executables[*Manage > System > Custom feeds > Malware signatures*], these signatures added by you will also be checked for any malicious software.
