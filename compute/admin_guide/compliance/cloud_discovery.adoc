...
They will be found if the credentials are created in the Compute Console under *Manage > Cloud accounts*.
endif::[]
...
=== Troubleshooting
Ensure you have the right permissions for the account before initiating cloud discovery. 

===== Empty results from Cloud Discovery

Cloud discovery results are visible per account. In case of multiple credentials belonging to the same account, the results are only displayed for one credential to avoid duplicacy. The other credentials of the same account will show empty results. To view comprehensive results from all credentials, navigate to Cloud Radar under Radars > Cloud tab.
