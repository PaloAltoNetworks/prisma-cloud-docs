...
. Select the accounts to scan. If there are no accounts in the table, you can onboard AWS/Azure/GCP accounts, using the *+ Add account* workflow.
...
..  Go to *Compute > Manage > Cloud Accounts* to view the scan report in tabular format.
...
..  Go to *Radar* and select *Cloud* to view the scan report in a visual format.
+
...
..  Scan reports can viewed under *Monitor > Vulnerabilities > {Hosts|Registry|Functions}*.
