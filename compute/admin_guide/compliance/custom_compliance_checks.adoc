...
Custom compliance checks are supported for Linux containers (docker or CRI-O), Windows containers (docker), and Linux Hosts. Custom compliance checks are not supported for Tanzu Application Service (TAS) defender.
...
By default, custom compliance checks for hosts are disabled.
...
Or if already deployed, xref:../install/deploy-defender/redeploy-defender.adoc[redeploy your Defenders].
