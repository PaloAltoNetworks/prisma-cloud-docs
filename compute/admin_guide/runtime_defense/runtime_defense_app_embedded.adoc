...
For more information, see xref:../install/install_defender/config_app_embedded_fs_protection.adoc[configuring the default setting for file system monitoring].
...
* *Changes to binaries or secret files* -- Detects when these types of files from a container image are modified.
...
+
NOTE: The *Prevent* effect isn't supported for this control.
