...
[NOTE]
====
* To interoperate, both the Console and the Jenkins plugin must be from the same release.

* The Jenkins plugin depends on the https://plugins.jenkins.io/matrix-project/[Matrix Project] plugin.
...
* While the Jenkins plugin doesn't support scanning Windows images for vulnerability and compliance issues on hosts with the containerd runtime, however, the Jenkins plugin does support scanning when running on hosts with Docker Engine.
