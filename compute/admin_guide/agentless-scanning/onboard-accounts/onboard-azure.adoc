...
. Select the *Security Coverage* you need. The *Agentless Scanning* and *Serverless Function Scanning* are enabled by default.

. Click *Next*.

. Provide the following information in the *Configure Account* page.

.. Enter he value of the `tenantId` field in the Azure output as the *Directory (Tenant) ID*.
