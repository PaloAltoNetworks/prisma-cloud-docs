...
The example target account uses the `example_target_project` name.
...
. If you encounter the `failed to create snapshot for instance ...` error or `Error 403: Required 'compute.disks.createSnapshot' permission for ...` error, use this https://docs.paloaltonetworks.com/content/dam/techdocs/en_US/pdf/prisma/prisma-cloud/prerelease/example-hub-target-permissions.tf.json[Terraform script] to grant the needed permissions across the target and hub accounts.

.. Edit the Terraform script to replace the `example_target_project` value in the `target_project_id` field with the project ID of the project your hub account scans.

.. Edit the Terraform script to replace the `example_hub_project` value in the `hub_project_id` field with the project ID of the account serving as a hub.
