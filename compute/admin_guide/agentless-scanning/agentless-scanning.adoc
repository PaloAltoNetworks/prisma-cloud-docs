...
=== Agentless Scanning of Serverless Functions

Prisma Cloud performs agentless scanning of serverless functions using a different process.
Agentless scanning performs the following steps to scan serverless functions.

. Validates that the Prisma Cloud role for the onboarded cloud account has the appropriate permissions and that those permissions are not blocked by an organizational policy.
. Identifies all serverless functions.
. Extracts the function using the appropriate `GET` method sending it to the Prisma Cloud console.
. Scans the function's code using Palo Alto Networks proprietary methods.
. Writes the scan results to the database of the Prisma Cloud console
