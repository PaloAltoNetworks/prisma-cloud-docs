...
====
NOTE
Enabling Agentless Scan on Azure with an https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/lock-resources[active scope lock] has a significant cost implication. The temporary resources that Prisma Cloud deploys for scanning cannot be deleted and continue to incur costs (VMs, disks, snapshots) because these resources are locked. The Prisma Cloud cosnole generates a log entrty that reports a _409 Conflict_ and _ERROR CODE: ScopeLocked_.
