...
Collections are created with xref:../configure/rule_ordering_pattern_matching.adoc[pattern matching] expressions that are evaluated against attributes such as image name, container name, hostname, labels, function name, namespace, and more.
...
NOTE: You cannot have collections that specify a combination of both:
...
+
image::collections_specify_filter.png[scale=15]
