...
. Select a runtime.
+
Prisma Cloud supports Lambda layers for *Node.js*, *Python*, *Ruby*, *C#*, and *Java*. See xref:../../system_requirements.adoc#serverless_runtimes[system requirements] for the runtimes that are supported for Serverless Defender as a Lambda layer.
