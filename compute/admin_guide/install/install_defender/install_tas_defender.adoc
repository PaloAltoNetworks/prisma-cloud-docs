...
Prisma Cloud automatically collects metadata fields such as Foundation, Organization Name, Application Name and ID and Space Name and ID. To utilize these fields, you'll have to **manually create** appropreiate xref:../../configure/collections.adoc[collections] that can then be used for filtering and aggregations.
