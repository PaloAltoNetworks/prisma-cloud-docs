...
You can now install the Prisma Cloud DaemonSet defender on your GKE *Autopilot* cluster. GKE autopilot clusters are using *containerd* nodes, hence the DaemonSet should be configured with CRI runtime. 

[. procedure]

. Generate the defender DaemonSet YAML or HELM chart, following the Google Kubernetes Engine (GKE) instractions

.. Make sure to set your enviroment runtime to CRI (not Docker):

... Set the *Nodes use Container Runtime Interface (CRI), not Docker* to ON when using the Console instractions
... twistcli, use the `--cri` option

. Make sure to create the *twistlock* namespace on your cluster 
     
     $ kubectl create namespace twistlock
  
. Remove the following mount `/var/lib/containers` from the YAML or Helm chart
...
... Open the downloaded tar.gz file and remove the following lines from 'daemonset.yaml' file:
...
... Install the new helm chart in your cluster

. Deploy the YAML or the Helm chart on your cluster

. After a few miniuts you should observ the nodes and running containers in the Console
