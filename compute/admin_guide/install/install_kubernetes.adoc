...
This procedure installs Prisma Cloud in your Kubernetes cluster quickly.
...
You can inspect, customize, and manage the YAML configuration files before deploying the Prisma Cloud Console and Defender.
Place the YAML configuration files under source control to track changes, to integrate them with CI/CD pipelines, and for teams to collaborate effectively. 

The Prisma Cloud Console is created as a `Deployment`, which ensures a single copy of the Prisma Cloud Console is always up and available.
Each Prisma Cloud Defender is deployed as a `DaemonSet`, which ensures that a Prisma Cloud Defender instance runs on each worker node in the cluster.

If a node goes down, the orchestrator should be able to simply reschedule the Prisma Cloud Console service on a different healthy node.
To improve the availability of the service, ensure that the orchestrator can run the service on any healthy node.
You can ensure this capability using the default YAML configuration files.
The default configuration enables the following features to ensure availability.
...
The Primsa Cloud Console service mmust always be accessible in your deployment because it serves a web interface and communicates policy with all the deployed Defenders.
...
To deploy your Defenders smothly, you must meet the following requirements.
...
The `twistcli` utility is included with every release, or you can <<_download_twistlock>>[download the utility].
...
. Create your first admininstrator.
