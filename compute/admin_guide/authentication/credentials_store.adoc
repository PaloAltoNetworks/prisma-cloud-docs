...
+
NOTE: When using organization-level credentials, the performance of several features may be affected if the organization includes a large number of projects. For specific information and guidelines see xref:../vulnerability_management/registry_scanning/scan_gcr.adoc[Google Container Registry scanning], xref:../compliance/cloud_discovery.adoc[Cloud Discovery], and xref:../install/install_defender/install_cluster_container_defender.adoc[Managed Defender DaemonSet deployment for GKE].
