...
Alert triggers:: 
Specifies when you want to send alerts to the alert provider included in the profile.
Alerts are generated when the rules included in your policy are violated, and you can choose whether you want to send notifications for issues. For example, runtime violations, compliance violations, cloud discovery or WAAS.
...
For Vulnerability, compliance, and cloud discovery alerts, the default frequency varies by integration and is enabled when you select the alert triggers for which you want to send notifications.
endif::prisma_cloud[]
...
When new vulnerabilities are detected on your resources, Prisma Cloud sends a vulnerabilities alert report every 24 hours. The report lists all new vulnerabilities that were detected in the past 24 hours.
...
+ 
You have the option to * Immediately alert for deployed resources*. 
Select this option, when an image with vulnerabilities is used to deploy a new host or container into your environment. This notification is applicable only when an image which has not been scanned previously is used to deploy a new resource.
...
For vulnerability, compliance, and cloud discovery the default frequency varies by integration and is enabled when you select the alert triggers for which you want to send notifications in step 4.
