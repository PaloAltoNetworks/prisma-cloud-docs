...
Add AWS SQS integration in Prisma Platform
...
+
image::alert-message-awssqs-json-body.png[width=300]

*Limitations*

* Additional SQS features such as associating attributes with the message, etc. can not be used as these capabilities are not supported by Prisma. In case of SQS, Compute sends the SQS message payload through Prisma API.
* Maximum allowed SQS message is 256kb. Messages larger than this limit won't be sent to Prisma, instead a specific error will be written to the Compue console log.
