...
NOTE: For the existing webhook alerts, you can edit custom JSON body and replace `#aggregated` macro with `#aggregatedAlerts` macro.
...
NOTE: For the existing webhook alerts, you can edit custom JSON body to add the `#dropped` macro.
...
|`#startupProcess` 
|The executed process activated when the container is initiated.
