...
Configure how Prisma Cloud connects to your cloud provider's managed Kubernetes service.
...
>>>> polling it every 10-15 minutes for new data -> not accurate we fetch every 10 minutes, but we'll fetch the data from up to 15 mins ago with a delay of 5 minutes.

No need to explain this to the customer but just to explain it, two options here:
1) A scan wasn't performed in the last 15 mins (when just enabling the feature): We'll fetch 15 mins of scan with a 5 minute delay (at 12:00 we fetch audits from 11:40 - 11:55)
2) A scan was performed in the last 10 minutes or less (the scheduled scan): then we fetch at 12:00 the audits from 11:45-11:55)
...
>>>> Stackdriver shouldn't be used in this context, it's a Google thing, not relevant to Azure
>>>> This section is not accurate, immediately doesn't work

Prisma Cloud supports only AKS clusters versions that allows logs exporting.  
...
.. Specify the Workspace Name. Recommended to use the free 7 days retention period workspace 
...
.. (Optional) Specify Advanced filter - specify filter to reduce the amount of data transferred
+ 
please use this reference https://docs.microsoft.com/en-us/azure/azure-monitor/logs/get-started-queries for queries syntax help  
...
NOTE: You will have to enable exporting EKS logs into AWS Cloud Watch enabling only cube audits, and Prisma Cloud will extract the logs from there. also there can be some delay between the time an event occurs in the cluster and when it appears in Cloud watch.


Due to Twistock's polling mechanism, there's another delay between the time an audit arrives in the Cloud Watch and it appears in Prisma Cloud.



Prisma Cloud supports only EKS clusters versions that allows logs exporting.  
...
=== Custom rules 
A custom rule is made up of one or more conditions. Configure custom rules policy, in order to trigger audits and match them.
...
you can filter by cluster name (applies to all cloude providers) project id(GCP), account id(AWS), resource group(only capital letters GCP), subscription id(Azure)
