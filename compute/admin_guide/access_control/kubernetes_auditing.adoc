...
On AKS, Prisma Cloud retrieves audits from "Log Analytics workspace", polling it every 10-15 minutes for new data.

NOTE: You will have to enable exporting AKS logs into Azure Workspace and Prisma Cloud will extract the logs from there. also there can be some delay between the time an event occurs in the cluster and when it appears in Workspace.
...
NOTE: For testing purposes, you might not want to wait for the 10 minute polling period to see audits in Prisma Cloud.
After setting up the integation in Prisma Cloud by providing your credentials, you can force Prisma Cloud to immediately poll Stackdriver by disabling then re-enabling the Kuberenetes audit feature in *Defend > Access > Kubernetes*.
...
.. Specify the workspace ID. Recommended to use the free 7 days retention period workspace 

.. Specify list resource groups. If unspecified, all resource groups will be used to retrieve the audits
...
[.task]
=== Integrating with EKS(Elastic Cloud Kubernetes)

On EKS, Prisma Cloud retrieves audits from AWS "Cloud watch", polling it every 10-15 minutes for new data.

NOTE: You will have to enable exporting EKS logs into Azure Cloud watch and Prisma Cloud will extract the logs from there. also there can be some delay between the time an event occurs in the cluster and when it appears in Cloud watch.
Due to Twistock's polling mechanism, there's another delay between the time an audit arrives in the Workspace and it appears in Prisma Cloud.
...
you can filter by cluster name (only capital letters)/ project id/ account id/ resource group/ subscription id
+
