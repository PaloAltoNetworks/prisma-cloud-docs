...
* Console and the Observers must be running the 22.06 version or later.

NOTE: With 22.12.xxx version, WAAS Agentless Observers will automatically upgrade to match the Console's version.
...
* Once the AWS setup is created/updated in agentless rule, the observer status is only available on *Manage > Defenders > Deployed defenders* page.
...
. Check if xref:../../agentless-scanning/onboard-accounts/onboard-aws.adoc[AWS account can connect with the Prisma Cloud Console] with the Console URL that you selected.
