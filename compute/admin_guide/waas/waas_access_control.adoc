...
NOTE: Country of origin is determined by the IP address associated with the request. When the X-Forwarded-For HTTP header is included in the request headers, Country of origin will be determined based on the first IP listed in the header value (true client IP).
