...
Apart from the scanning the default Prisma Cloud policies, Checkov also scans for policy violations in custom policies that you can configure during xref:../../get-started/code-repositories-policy-management/custom-build-policy-examples.adoc[custom build-time checks.]
...
+
See https://docs.paloaltonetworks.com/prisma/prisma-cloud/prisma-cloud-admin/get-started-with-prisma-cloud/enable-access-prisma-cloud-console.html#id7cb1c15c-a2fa-4072-%20b074-063158eeec08[enable access to the Prisma Cloud Console] for what IP addresses and hostnames for Application security need to be allowed for successfully integrating with Prisma Cloud. 
