...
. Optionally, enable the xref:../code-security-licensing-configuration.adoc[CI/CD Security Module] for enhanced Protection in your Azure Repos Environment.
+
image::azure-cicd-int.png[width=600]

.. Add *User Name*.
+
NOTE: To retrieve your user name, access Azure organization and then select *User settings > Profile*.

.. Access Azure Repos to generate a xref:https://learn.microsoft.com/en-us/azure/devops/organizations/accounts/use-personal-access-tokens-to-authenticate?view=azure-devops&tabs=Windows[Personal Access Token] (PAT) and make a copy for safekeeping.
+
To create a new token using the values here.
+
* Select *Organization > all accessible organizations* and set an *Expiration date*.
...
.. Access Prisma Cloud console and add the generated PAT in the *App Password* and then select *Next*.
