...
As a prerequisite, ensure Prisma Cloud IP addresses are on the allow list for Application Security. For more details see https://docs.paloaltonetworks.com/prisma/prisma-cloud/prisma-cloud-admin/get-started-with-prisma-cloud/enable-access-prisma-cloud-console[enable access to the Prisma Cloud IP addresses and hosts].
