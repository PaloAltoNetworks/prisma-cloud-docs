...
[IMPORTANT]
====
Pipeline Tools currently supports Jenkins declarative pipelines, CircleCI, GitLab CI and GitHub Actions. 
====
...
[NOTE]
==== 
When ‘Not available’  status is displayed, it indicates that the tool is not in Prisma Cloud’s Pipeline Tools catalog. This could be because it is either an internal tool or a public tool which will be added to the catalog in the near future.
====
